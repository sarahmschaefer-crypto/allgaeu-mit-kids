...
- **Journey/Pfad-Animation** war wiederholt fiddlig → wenn es wieder hakt: einen **einfacheren, robusten Ansatz vorschlagen** statt weiter am SVG-`dashoffset` zu feilen.
...
- **Typ-Check:** `npx tsc --noEmit`
- **Lint:** `npm run lint`
- **Keine automatisierten Tests** im Projekt — Verifikation läuft visuell über localhost.
...
- **Schriften:** Playfair Display, Nunito, JetBrains Mono — alle via Google Fonts `<link>` in `app/layout.tsx` (kein Next.js Font-Loader).
- **Remote-Bilder:** Unsplash (`images.unsplash.com`) ist in `next.config.ts` freigeschaltet.
