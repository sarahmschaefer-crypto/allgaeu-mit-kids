...
- **Cover-Builder (separates Teilprojekt, `cover-tool`-Branch):** `lib/cover/` (Renderer-Spec, presets, figma-templates) + `components/cover/` + `app/admin/`. Schichten-Modell (Foto → Rahmen → Scrim → Brand-Grafik → Kategorie-Stempel → Schrift), ein Renderer.
